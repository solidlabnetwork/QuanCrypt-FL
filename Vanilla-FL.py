...
csv_filename = args.csv_filename if args.csv_filename else (
    ...
    f"clip_factor({clip_factor})-lambda_param({lambda_param})-clients({n_clients}).csv"
)
...
# Create dataloaders
# client_dataloaders = [DataLoader(DatasetSubset(full_dataset, dict_users[i]), batch_size=batch_size, shuffle=True) for i in range(n_clients)]
...
for i in range(n_clients):
    if len(dict_users[i]) > 0:
        client_dataloaders.append(
            DataLoader(DatasetSubset(dataset_for_partition, dict_users[i]), batch_size=batch_size, shuffle=True)
        )
        valid_clients.append(i)
    else:
        print(f"[WARNING] Skipping Client {i}: No data assigned.")

# client_dataloaders = [DataLoader(DatasetSubset(dataset_for_partition, dict_users[i]), batch_size=batch_size, shuffle=True) for i in range(n_clients)]

val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)

# Display client sample counts
for i, dataloader in enumerate(client_dataloaders):
    total_samples = sum(len(batch[0]) for batch in dataloader)
    print(f"Client {i + 1} has {total_samples} training samples.")

# Model initialization
models_dict = {'CNNMNIST': CNNMNIST, 'AlexNet': AlexNet,'CNNHAM64':CNNHAM64, 'ResNet18': ResNet18}
model_type = models_dict.get(model_name)
global_model = model_type().to(device) if model_type else None

# Checkpoints
checkpoint_path1 = os.path.join(checkpoint_dir, f'QuanCryptFL-{model_name}-{args.dataset}-{args.batch_size}-{args.partition}-clip_factor({clip_factor})-lambda_param({lambda_param})-clients({n_clients})_1.pth')
checkpoint_path2 = os.path.join(checkpoint_dir, f'QuanCryptFL-{model_name}-{args.dataset}-{args.batch_size}-{args.partition}-clip_factor({clip_factor})-lambda_param({lambda_param})-clients({n_clients})_2.pth')
...
for round in range(start_round, num_rounds):
    ...
    for i in range(n_clients):
        ...
        client_update = client_train(args=args, device=device, lr=learning_rate, weight_decay=weight_decay, dataloader=client_dataloaders[i])
