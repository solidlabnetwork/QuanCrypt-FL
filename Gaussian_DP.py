...
import math
from utils.Nets import ResNet18
...
checkpoint_path = os.path.join(checkpoint_dir, f'checkpoint_Gaussian_eps_{epsilon}_1.pth')
checkpoint_path2 = os.path.join(checkpoint_dir, f'checkpoint_Gaussian_eps_{epsilon}_2.pth')
...
for round in range(start_round, num_rounds):
    ...
    for i in range(n_clients):
        ...
        for key in c_r_keys:
            ...
            noise = torch.normal(mean=0, std=sigma, size=param_data.shape).to(device)
...
filename = f"{output_dir}/Gaussian_DP_FL_{model_name}_{epsilon}_{lambda_param}_{n_clients}.xlsx"
